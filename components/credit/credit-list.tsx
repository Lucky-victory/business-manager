...
export function CreditList() {
  ...
  const { credits, fetchCredits, debtors, getTotalOutstandingCredit } = useStore()
  ...
  const creditsByDebtor = filteredCredits.reduce(
    (acc, credit) => {
      ...
      if (credit.type === "purchase") {
        acc[credit.debtorId].totalAmount += credit.amount

        // Track paid vs unpaid amounts
        if (credit.isPaid) {
          acc[credit.debtorId].paidAmount += credit.amount
        } else {
          acc[credit.debtorId].unpaidAmount += credit.amount
        }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Total Outstanding: <span className="font-medium text-red-500">${totalOutstandingCredit.toFixed(2)}</span>
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => setIsReportOpen(true)}>Generate Report</DropdownMenuItem>
              <DropdownMenuItem onClick={() => router.push("/credit/summary")}>View Summary</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          >
            ...
            <CardContent>
              <div className="flex justify-between">
                ...
                <span className={`font-medium ${debtorCredit.totalAmount > 0 ? "text-red-500" : "text-green-500"}`}>
              ...
              {statusFilter === "all" && (
                <>
                  <div className="flex justify-between mt-2">
                    ...
                    <span className="font-medium text-green-500">${debtorCredit.paidAmount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between mt-2">
                    <span className="text-muted-foreground">Unpaid Amount:</span>
                    <span className="font-medium text-red-500">${debtorCredit.unpaidAmount.toFixed(2)}</span>
                  </div>
                </>
              )}
              <div className="flex justify-between mt-2">
                <span className="text-muted-foreground">Last Update:</span>
                <span className="font-medium">{format(new Date(debtorCredit.lastUpdate), "MMM d, yyyy")}</span>
              </div>
            </CardContent>
