...
export default function SalesDetailPage({ params }: { params: { date: string } }) {
  ...
  const formattedDate = dateString ? format(new Date(dateString), "MMMM d, yyyy") : "Loading..."
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <p className="text-muted-foreground">
            Total Sales: <span className="font-medium text-foreground">${totalAmount.toFixed(2)}</span>
          </p>
          <p className="text-muted-foreground">
            Items Sold: <span className="font-medium text-foreground">{filteredSales.length}</span>
          </p>
        </div>
      ...
      <div className="space-y-4">
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{format(new Date(sale.date), "h:mm a")}</p>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {filteredSales.length === 0 && (
          <div className="text-center py-10 text-muted-foreground">No sales recorded for this date.</div>
        )}
      </div>
